...
" move favorite file suffixes to top
" explFavSuffix is a comma separated list of suffixes to move up
" like .c,.h
" explFavSuffixFirst == 1 - moves suffixes to top
" explFavSuffixFirst == -1 - moves suffixes to bottom of the listing
if !exists("g:explFavSuffix")
  let g:explFavSuffix=''
  let g:explFavSuffixFirst=0
endif
...
function s:SetSuffixesFav() 
	let b:suffixesFav = '\(' . substitute(escape(g:explFavSuffix,s:escregexp),',','\\|','g') . '\)$'
	if has("fname_case")
		let b:suffixesFav = '\C' . b:suffixesFav
	else
		let b:suffixesFav = '\c' . b:suffixesFav
	endif
endfunction
...
function! s:FileNameCmp(line1, line2, direction)
	...
	elseif (g:explSuffixesLast != 0) && (f1 !~ b:suffixesRegexp) && (f2 =~ b:suffixesRegexp)
		return -g:explSuffixesLast
	elseif (g:explFavSuffixFirst != 0) && (f1 =~ b:suffixesFav) && (f2 !~ b:suffixesFav)
		return -g:explFavSuffixFirst
	elseif (g:explFavSuffixFirst != 0) && (f1 !~ b:suffixesFav) && (f2 =~ b:suffixesFav)
		return g:explFavSuffixFirst
	else
...
"   echo a:msg . "Sorting by" . w:sortdirlabel . w:sorttype
    ...
    " Create a regular expression out of the explFavSuffix list in case
    " we need it.
    call s:SetSuffixesFav()
