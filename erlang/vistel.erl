...
%    end.
	...
	{ok, BeamFile} ->
		...
		{ok, Exports0} ->
			...
			Fns = [Fun || {Fun, _Arity} <- Exports, lists:prefix(Prefix, Fun) ],
		    {ok, ordsets:to_list(ordsets:from_list(Fns))};
		error ->
...
records_list(FileOrModule, Pref) ->
	Res = read_records(FileOrModule, []),
	{ok, [ Name || {attribute, _, record, {Name, _}} <- Res, lists:prefix(Pref, atom_to_list(Name)) ]}.

record_fields(FileOrModule, Record, Pref) ->
	Res = read_records(FileOrModule, []),
	RecordAtom = list_to_atom(Record),
	[Row] = [ Val || {attribute, _, record, {Name, Val}} <- Res, Name == RecordAtom ],
	Result = [ element(3, element(3, X)) || 
		       X <- Row, 
					element(1, X) == record_field, 
					lists:prefix(Pref, atom_to_list(element(3, element(3, X))))
			 ],

	{ok, Result}.

%read_records(FileOrModule) ->
%	Res = read_records(FileOrModule, []),
...
find_file(File) ->
	ModName = list_to_atom(filename:rootname(filename:basename(File))),
	case code:which(ModName) of
	L when is_list(L) -> find_file(ModName);
	_ ->
